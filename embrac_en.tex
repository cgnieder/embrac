...
squared brackets? Or curly braces, too? This can be done with these commands:
\begin{beschreibung}
...
You can change them as you wish, but be careful with the kerning! What's to less
...
Deleting a pair has the expected results:
\begin{beispiel}
 \DeleteEmph()
 \emph{This is emphasized [sic] text.} \\
 \emph{This is emphasized text (as you can see).}
...
\begin{beschreibung}
 ...
 and squared brackets as defined by \cmd{bibleftparen}, \cmd{bibrightparen},
 \cmd{bibleftbracket} and \cmd{bibrightbracket} are treated the same way (if not
 removed from the treatment with \cmd{DeleteEmph}).
\end{beschreibung}
...
This document for example uses the following lines:
\begin{beispiel}[code only]
 \EmbracOff
 \printbibliography
\end{beispiel}
